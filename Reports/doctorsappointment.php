...
$user_id = $_SESSION['user_id']; // Get logged-in doctor's user ID

// Call the stored procedure with doctor's user ID
$stmt = $conn->prepare("CALL doctorsappointment_rp(?)");
...
while ($row = $result->fetch_assoc()) {
    if (empty($search_term) || 
        ...
        stripos($row['URL'], $search_term) !== false) {
              ...
              <form method="GET" action="" class="row g-2 align-items-center mb-4">
                <div class="col-md-6">
                  <div class="input-group">
                    ...
                    <input type="text" class="form-control" name="search" placeholder="Search my appointments..." 
                    ...
                    <tbody>
                      <?php foreach ($filtered_results as $appointment): ?>
                        <tr>
                          ...
                          <td>
                            <?php if (!empty($appointment['URL'])): ?>
                              <a href="<?= htmlspecialchars($appointment['URL']) ?>" target="_blank" class="btn btn-sm btn-outline-primary join-meeting-btn">
          ...
          <div class="modal fade" id="editAppointmentModal" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-lg modal-lg-custom">
              <div class="modal-content">
                ...
                <form id="editAppointmentForm" method="POST" action="../updates/uproveappointment.php">
                  ...
                  <div class="modal-body">
                    ...
                    <div class="row mb-3">
                      ...
                      <div class="col-md-4 mb-3">
                        <label for="editURL" class="form-label">Meeting URL</label>
                        <div class="position-relative">
                          <i class="bi bi-link form-icon"></i>
                          <input type="url" id="editURL" name="url" class="form-control form-icon-input">
                        </div>
                      </div>
                    </div>
                  </div>
  ...
  <script>
    ...
    document.querySelectorAll('.edit-btn').forEach(btn => {
      btn.addEventListener('click', function() {
        ...
        document.getElementById('editAppointmentId').value = appointmentData.appointment_id;
        ...
        document.getElementById('editURL').value = appointmentData.URL || '';
        
        // Set modal title
        document.querySelector('#editAppointmentModal .modal-title').innerHTML = 
          `<i class="bi bi-calendar2-event me-2"></i> Edit Appointment #${appointmentData.appointment_id}`;
      });
    });
    ...
    function reloadAppointmentsTable() {
      ...
      fetch('doctorsappointment.php?' + params.toString(), { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
        .then(res => res.text())
        .then(html => {
